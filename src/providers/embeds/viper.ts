...
export const viperScraper = makeEmbed({
  ...
  async scrape(ctx) {
    ...
    const proxiedPlaylist = `https://proxy.fithwit.net/m3u8-proxy?url=${encodeURIComponent(playlistUrl)}&headers=${encodeURIComponent(JSON.stringify({ Referer: 'https://megacloud.store/' }))}`;
